...
func printDeviceInfo(device common.Device) {
	...
	boxStyle := lipgloss.NewStyle().Border(lipgloss.NormalBorder()).BorderForeground(lipgloss.Color("#51e2f5"))
	...
	privateSessionStyle := valueStyle.Render(privateSessionSymbol)
	...
	typeStyle := valueStyle.Render(typeSymbol)
	...
	formattedInfo := fmt.Sprintf(
		"Device Name       : %s\n"+
			...
			"Type              : %s %s\n"+
			"Volume Percent    : %d%% %s\n",
		valueStyle.Render(device.Name),
		activeStatusStyle.Render(activeStatusSymbol),
		valueStyle.Render(fmt.Sprintf("%v", device.IsActive)),
		valueStyle.Render(device.ID),
		privateSessionStyle,
		valueStyle.Render(fmt.Sprintf("%v", device.IsPrivateSession)),
		valueStyle.Render(fmt.Sprintf("%v", device.IsRestricted)),
		valueStyle.Render(fmt.Sprintf("%v", device.SupportsVolume)),
		typeStyle,
		valueStyle.Render(device.Type),
		device.VolumePercent,
		volumeEmojis,
	)

	// Combine header and formatted info inside a box
	fullBox := boxStyle.Render(headerStyle.Render("         Device Information          ") + "\n" + formattedInfo + "\n")
