...
func SpotifySearchResultsPrompt(body []byte) string {
	...
	formattedInfo := make([]string, len(response.Tracks.Items))
	...
	prompt := promptui.Select{
		Label: "Select track",
		Items: formattedInfo,
		Size:  len(response.Tracks.Items),
	...
	selectedTrack := response.Tracks.Items[index]
	formattedSongInfo := fmt.Sprintf(
		...
		//common.ValueStyle.Render(selectedTrack.Popularity),
