...
func UserReadTokenHandler(w http.ResponseWriter, r *http.Request) {
	...
	accessToken, expiresIn, err := auth.FetchAuthToken(config.GlobalConfig.ClientId, config.GlobalConfig.ClientSecret, authCode, constants.ServerUrl+constants.DeviceCallBackRoute)
